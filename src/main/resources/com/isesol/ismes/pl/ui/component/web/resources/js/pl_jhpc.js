...
angular.module('angularGanttDemoApp')
    .controller('MainCtrl', ['$scope', '$timeout', '$log', '$filter', 'ganttUtils', 'ganttMouseOffset', 'ganttDebounce', 'moment', '$http', function($scope, $timeout, $log, $filter, utils, mouseOffset, debounce, moment, $http) {
        ...
        // 初始化排产三个参数的值
        var pcsl = parseInt(ui.hidden('pcsl').val());
        $scope.per = parseInt(ui.hidden('zxsl').val());
        $scope.min = pcsl < 10 ? 1 : (Math.floor(pcsl / 10) > 3 ? 3 : Math.floor(pcsl / 10));
        $scope.max = pcsl < 10 ? 1 : Math.ceil(pcsl / 10);
        ...
        var handleTaskEvent = function(eventName, task) {
            ...
            if (eventName == 'tasks.on.resizeEnd' || eventName == 'tasks.on.moveEnd') {
            	...
            	if(!checkTask(task)) {
            		...
            		currentDeviceTask.xh = task.model.xh;
        ...
        $scope.options = {
            ...
            columnsHeaders: {'model.gx': '工序组'},
            treeHeaderContent: '加工单元',
            autoExpand: 'none',
        ...
        var popMenu = function(event, task) {
        	$scope.editedTask = task.clone();
        	document.getElementById("contextMenu").style.top = (event.pageY - $(".container")[2].offsetTop - 100)+ 'px';
        ...
        var checkGx = function(gxsbids) {
        	for (var i = 0; i < backupGx.length; i++) {
        		...
        		if (!found) {
        			ui.alert(backupGx[i].gxmc + "下至少需要有一台加工单元被选中");
        			return false;
        		}
        	}
        	return true;
        ...
        $scope.trySchedule = function() {
        	...
        	if ($scope.per == 0 || $scope.per === undefined) {
        		ui.alert('装箱数量不能为0');
        		return;
        	}
        	if ($scope.min == 0 || $scope.min === undefined) {
        		ui.alert('工单最小单位数量不能为0');
        		return;
        	}
        	if ($scope.max == 0 || $scope.max === undefined) {
        		ui.alert('工单最大单位数量不能为0');
        		return;
        	}
        	if ($scope.min > $scope.max) {
        		ui.alert('工单最大单位数量不能小于工单最小单位数量');
        		return;
        	}
        	var loading = new ol.loading({id:"ganttAppDiv"});
    		loading.show();
             ...
             .then(function successCallback(response) {
            	 if (response.data.error !== undefined) {
            		 loading.hide();
            		 ui.warn(response.data.error, "试排失败!");
            		 return;
            	 }
            	 $timeout(function() {
                  	$scope.api.side.setWidth(undefined);
        ...
        $scope.save = function(type) {
        		...
        		if ($scope.data[i].drawTask && tasks !== undefined) {
        			...
        			for (var j = 0; j < tasks.length; j++) {
        				if(tasks[j].pcid !== undefined && tasks[j].pcid == ui.hidden('pcid').val()) {
        					...
        					if (j == 0)
    						{
        						tasks[j].hxgd = "unknown";
    						} else {
    							tasks[j].hxgd = "unknown";
            					for (var k = j - 1; k >= 0; k--) {
            						if (tasks[k].pcid === undefined) {
            							// 前一个是设备占用
            							continue;
            						}
            						if (tasks[k].pcid !== undefined) {
            							// 找到最近的工单
            							if (tasks[k].ljid == tasks[j].ljid)
        								{
            								tasks[j].hxgd = "no";
        								} else {
        									tasks[j].hxgd = "yes";
        					...
        					var hxgd = tasks[j].hxgd;
        					if (tasks[j].xh === undefined || tasks[j].xh == '') {
        						ui.error("加工单元‘" + $scope.data[i].label + "’上有箱号为空的工单，请填写箱号");
        						savings = {};
        						return;
        					}
        					var xh = tasks[j].xh;
        					...
        					savings["gd"].push({"gxid":gxid,"xh":xh, "sbid":sbid, "zzjgid":zzjgid, "jgsl":jgsl, "jhkssj":jhkssj, "jhjssj":jhjssj, "fgdbh":fgdbh, "gdid":gdid, "gdbh":gdbh , "zbsj":zbsj, "gxzxh":gxzxh, "gxzbs":gxzbs, "hxgd":hxgd,"jgsj":jgsj});
        				}
        			}
        		}
        	}
        	...
        	ui.confirm("你确定要" + prompt + "本批次所有工单吗？", function(req){
				if(req){
		        	$http({
						method:'POST',
						url:'saveSchedule',
        ...
        var recalculate = function(isFromDelete) {
        	...
        	for (var i = 0; i < $scope.data.length; i++) {
        		// 刷新工序信息
        		if ($scope.data[i].gx !== undefined && $scope.data[i].gx != '' && $scope.data[i].gx != null) {
