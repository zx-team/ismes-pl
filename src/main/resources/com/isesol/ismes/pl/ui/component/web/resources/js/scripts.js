...
angular.module('angularGanttDemoApp', [
    ...
    'ngAnimate',
    'mgcrea.ngStrap'
]).config(['$compileProvider','cfpLoadingBarProvider', function($compileProvider, cfpLoadingBarProvider) {
...
angular.module('angularGanttDemoApp')
    .controller('MainCtrl', ['$scope', '$timeout', '$log', '$filter', 'ganttUtils', 'ganttMouseOffset', 'ganttDebounce', 'moment', '$http', 'cfpLoadingBar', function($scope, $timeout, $log, $filter, utils, mouseOffset, debounce, moment, $http, cfpLoadingBar) {
    	// 零件图片
        ...
        var toDay = new Date();
        fromDay.setDate(today.getDate() - 1); // 前1天
        toDay.setDate(today.getDate() + 7);  // 后7天
        
        $scope.fromDayTemp = new Date();
        $scope.toDayTemp = new Date();
        $scope.fromDayTemp.setDate(today.getDate() - 1); // 前1天
        $scope.toDayTemp.setDate(today.getDate() + 7);  // 后7天
        ...
        var popMenu = function(event, task) {
        	$scope.editedTask = task.clone();
        	document.getElementById("contextMenu").style.top = (event.pageY - $(".container")[3].offsetTop - 150)+ 'px';
        	document.getElementById("contextMenu").style.left = (event.pageX - $(".container")[3].offsetLeft) + 'px';
        ...
        function disableButtons() {
        	ui.button("trySchedule").attr("disabled","true");
    		ui.button("releaseSchedule").attr("disabled","true");
    		ui.button("saveSchedule").attr("disabled","true");
        }

        // Reload data action
        $scope.load = function() {
        	if (parseInt(ui.hidden('pcjhztdm').val()) >= 40) {
        		disableButtons();
        	}
        	 $http({
				method:'GET',
        ...
        $scope.save = function(type) {
        	...
        	if (type == 'save') {
        		...
        		prompt = '发布';
//        		ui.confirm("是否发布", function(data){
//    				if(!data){ return; }
//    				});
        		if(!window.confirm('你确定要发布本批次所有工单吗？')){
    				return;
    			}
        	}
        		...
        		if ($scope.data[i].drawTask && tasks !== undefined) {
        			...
        			for (var j = 0; j < tasks.length; j++) {
        				if(tasks[j].pcid !== undefined && tasks[j].pcid == ui.hidden('pcid').val()) {
        					...
        					savings["gd"].push({"gxid":gxid, "sbid":sbid, "jgsl":jgsl, "jhkssj":jhkssj, "jhjssj":jhjssj, "fgdbh":fgdbh, "gdid":gdid, "gdbh":gdbh , "zbsj":zbsj});
        				}
        			}
        		}
        	}
        	...
        	$http({
             ...
             .then(function successCallback(response) {
            	 ui.success('工单' + prompt + '成功');
            	 $scope.pcjhztmc = response.data.pcjhztmc;
            	 if (prompt == '发布') {
            		 disableButtons();
            	 }
            	 // 重新加载甘特图数据
        ...
        var recalculate = function() {
        	...
        	var totalTime = 0;
        	if ($scope.maxTo !== undefined) {
        		totalTime = $scope.maxTo.diff($scope.minFrom)
        	}
        	// 计算设备利用率
        	for (var i = 0; i < $scope.data.length; i++) {
        		var tasks = $scope.data[i].tasks;
        		if ($scope.data[i].drawTask && tasks !== undefined) {
        			...
        			for (var j = 0; j < tasks.length; j++) {
        				...
        				if ($scope.minFrom.isBetween(fromStr, toStr) && $scope.maxTo.isBetween(fromStr, toStr) ) {
        					occupyTime = totalTime;
        					break;
        				}
        				if (to.isBefore($scope.minFrom) || to.isSame($scope.minFrom)) {
        					continue;
        				}
        				if (from.isAfter($scope.maxTo) || from.isSame($scope.maxTo)) {
        					continue;
        				}
        				if ($scope.minFrom.isBetween(fromStr, toStr)) {
        					occupyTime += to.diff($scope.minFrom);
        					continue;
        				}
        				if ($scope.maxTo.isBetween(fromStr, toStr)) {
        					occupyTime += $scope.maxTo.diff(from);
        					continue;
        				}
